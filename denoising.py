...
def segment_sum(data: torch.Tensor, segment_ids: torch.Tensor) -> torch.Tensor:
    ...
    result = torch.zeros(max_segment_id, *data.shape[1:], dtype=data.dtype, device=data.device)
...
def unsorted_segment_sum(data: torch.Tensor, segment_ids: torch.Tensor, num_segments: int) -> torch.Tensor:
    ...
    """
    assert data.shape[0] == segment_ids.shape[0], "data and segment_ids must have the same size in the first dimension"
...
def random_spans_noise_mask(length, noise_density, mean_noise_span_length=3.0):
    ...
    def _random_segmentation(num_items, num_segments):
        first_in_segment = torch.nn.functional.pad(
            stateless_shuffle((torch.arange(num_items - 1) < num_segments - 1).to(torch.long)),
            (1, 0),
        )
    ...
    interleaved_span_lengths = torch.stack([nonnoise_span_lengths, noise_span_lengths], dim=1).reshape(num_noise_spans * 2)
    span_starts = torch.cumsum(interleaved_span_lengths, dim=0)[:-1]
    span_start_indicator = unsorted_segment_sum(torch.ones_like(span_starts), span_starts, length)
...
def random_prefix_noise_mask(length, noise_density):
    ...
    min_input_tokens = min(max_input_tokens, max(1, round((1 - 2 * noise_density) * max_input_tokens)))
...
def nonnoise_span_to_fixed_sentinel(tokens, noise_mask, sentinel_value):
    return noise_span_to_fixed_sentinel(tokens, torch.logical_not(noise_mask), sentinel_value)
